...
var debug_log = require("./debuglog");
...
module.exports = {
  ...
  guess_progression_labelings: function(chord_list) {
    if (typeof(chord_list) === 'string') {
      chord_list = get_chords_from_str(chord_list);
    }
  ...
  main: function(chords) {
    var progressions = [];
    var chord_list = get_chords_from_str(chords);

    var likely_progressions = module.exports.guess_progression_labelings(chord_list);
    ...
    progressions  = _.sortBy(progressions, function(p) {
      p.likeliness = module.exports.decide_progression_likeliness(p.labeling.slice(0, 8));
      return -p.likeliness;
    });
    ...
    _.each(progressions, function(progression) {
      ...
      var modulations = module.exports.get_possible_modulations(progression);
    ...
    var sorted_progressions = _.sortBy(progressions, function(p) {
      debug_log("KEY", p.key, "HARMONIOUSNESS", module.exports.get_progression_harmoniousness(p.mod_labeling), "BREAKS", module.exports.check_progression_grammar(p.mod_labeling));
      p.likeliness = module.exports.decide_progression_likeliness(p.labeling);
      p.mod_likeliness = module.exports.decide_progression_likeliness(p.mod_labeling);
    ...
    debug_log("USING KEY", sorted_progressions[0].key);
    debug_log("HARMONIOUSNESS", module.exports.get_progression_harmoniousness(prog.mod_labeling), "BREAKS", module.exports.check_progression_grammar(prog.mod_labeling));
